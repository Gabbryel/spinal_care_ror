...
export default class extends Controller {
  ...
  connect() {
    // Only initialize animation elements if they exist (public site navbar)
  ...
  toggle() {
    ...
    // Only do hamburger animations if we have those targets (public site)
    if (this.hasHamburgerTarget) {
    ...
    if (
      this.hasHamburgerTarget &&
      this.hamburger.classList.contains("closed")
    ) {
  ...
  close() {
    ...
    // Only do hamburger animations if we have those targets
    if (this.hasHamburgerTarget) {
    ...
    // Only do animations if we have the hamburger target
    if (this.hasHamburgerTarget) {
      setTimeout(() => {
        this.longLine2.classList.remove("active-long-line-2");
      }, 10);
